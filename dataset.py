...
def load_data(img_gt_path, train):
    ...
    points = []
    try:
        mat_data = scipy.io.loadmat(gt_path)  # Read .mat file
        if 'annPoints' in mat_data:  # Check for expected key
            points = mat_data['annPoints']  # Load annotation points
        elif 'annotations' in mat_data:
            points = mat_data['annotations']
        else:
            print(f"Warning: No valid annotation key found in {gt_path}")
    ...
    if isinstance(points, np.ndarray):
        ...
        print(f"Warning: Points in {gt_path} are not in expected format.")
        return img, np.empty((0, 2))  # Return empty array if no valid points
