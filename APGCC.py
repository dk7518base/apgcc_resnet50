...
class Model_builder(nn.Module):
    def __init__(self, cfg):
        ...
        self.to(self.device)  # 모델을 GPU 또는 CPU로 이동
...
#        return encoder
    def _build_encoder(self):
        ...
        # input: image, output: [feat1(H/2,W/2), feat2(H/4,W/4), feat3(H/8,W/8), feat4(H/16,W/16)]
        #########################################################################################
        print(f"Selected Encoder: {self.cfg.MODEL.ENCODER}")  # 디버깅용 출력

        # 조건에 따라 올바른 Encoder를 가져옵니다.
        if self.cfg.MODEL.ENCODER in ['vgg16', 'vgg16_bn']:
            from .Encoder import Base_VGG as build_encoder
        elif self.cfg.MODEL.ENCODER in ['resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152','resnet50_ibn_a']:
            from .Encoder import Base_ResNet as build_encoder
        else:
            raise ValueError(f"Unsupported encoder: {self.cfg.MODEL.ENCODER}")

        # ENCODER_kwargs에서 'name' 키를 제거
        encoder_kwargs = self.cfg.MODEL.ENCODER_kwargs.copy()  # 원본을 보존
        encoder_kwargs.pop('name', None)

        #self.cfg.MODEL.ENCODER_kwargs['name'] = self.cfg.MODEL.ENCODER
        
        # build_encoder 함수에 필요한 파라미터 전달
        encoder = build_encoder(
            name=self.cfg.MODEL.ENCODER,    # name 인자 추가
            **self.cfg.MODEL.ENCODER_kwargs # 나머지 인자 추가
        )        
        print("Encoder successfully built")  # 확인용 출력
        return encoder   
