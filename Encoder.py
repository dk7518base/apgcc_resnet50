...
#         # Extract pretrained and path from kwargs
...
class Base_ResNet(nn.Module):
    def __init__(self, name: str, last_pool=False, num_channels=256, **kwargs):
        ...
        from .backbones import resnet as models
        if name == 'resnet18':
            self.backbone = models.resnet18_ibn_a(pretrained=False)  # pretrained=False로 초기화
        elif name == 'resnet34':
            self.backbone = models.resnet34_ibn_a(pretrained=False)
        elif name == 'resnet50':
            self.backbone = models.resnet50_ibn_a(pretrained=False)
        elif name == 'resnet101':
            self.backbone = models.resnet101_ibn_a(pretrained=False)
        elif name == 'resnet152':
            self.backbone = models.resnet152_ibn_a(pretrained=False)
        elif name == 'resnet50_ibn_a':  # 추가 확인
            self.backbone = models.resnet50_ibn_a(pretrained=False)      
        else:
        ...
        if pretrained and path:
            print(f"Loading pretrained weights from {path}")
            state_dict = torch.load(path, map_location='cpu')
            self.backbone.load_state_dict(state_dict, strict=False)  # strict=False로 로드
            print("Pretrained weights loaded successfully!")
